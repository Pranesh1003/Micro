...
// Helper to resolve login page URL dynamically, keeping subdirectory context (like /micro/frontend/) on port 63342
function getLoginUrl() {
    const path = window.location.pathname;
    const marker = '/frontend/';
    const index = path.indexOf(marker);
    if (index !== -1) {
        return path.substring(0, index + marker.length) + 'auth/login/index.html';
    }
