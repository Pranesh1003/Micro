...
public class BookingServiceImpl implements BookingService {
    ...
    public List<Flight> searchAvailableFlights(Booking searchCriteria) {
        ...
        List<Flight> flights = flightClient.getAllFlights().stream()
    ...
    private Flight resolveFlightReference(Booking booking) {
        ...
        Flight flight = flightClient.getFlightById(resolvedFlightId);
        if (flight == null) {
            throw new IllegalArgumentException("Flight not found for id: " + resolvedFlightId);
        }
    ...
    private void awardDistanceMiles(Booking booking) {
        try {
            ...
            Flight flight = flightClient.getFlightById(booking.getFlightId().longValue());
